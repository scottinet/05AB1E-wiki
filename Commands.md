...
`ʒ` | `a` | filter a when the result of code == 1: usage ʒCODE}
...
`#` | `a` | if contains spaces, split on spaces
`%` | `a,b` | push (a % b)
`&` | `a,b` | push a AND b
`(` | `a` | push -a
`*` | `a,b` | push (a * b)
...
`@` | `a` | pop and push the element at index a in the stack (leftmost element = index 0)
`B` | `a,b` | push base(a, b)
`C` | `a` | push int(a, 2)
`D` | `a` | push a, a
...
`°` | `a` | push 10 ** a
`±` | `a` | push bitwise not a
`µ` | `a` | while counter_variable != a, do...
`·` | `a` | push 2 * a
`¸` | `a` | push [a]
`»` | `(a)` | if list, join list by newlines, else join stack by newlines
`½` | `a,` | if 1, then counter_variable += 1
`¿` | `a` | push gcd(a)
...
`Ê` | `a,b` | push a != b
`Ë` | `a` | push 1 if all equal else 0
`Ì` | `a` | push a + 2
`Í` | `a` | push a - 2
...
`Ø` | `a` | push ath prime (zero-indexed)
...
`ã` | `a,b` | push a choose b (cartesian product repeat)
